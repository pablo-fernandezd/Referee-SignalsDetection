...
const ManualCrop = ({ imageFile, onSubmit }) => {
  ...
  const handleSubmit = () => {
    ...
    if (!selectedClassObj || selectedClassObj.id === -1) { // If 'none' selected
      ...
      onSubmit({ bbox: [], class_id: -1 }); // Send empty bbox and -1 for 'none'
      return;
    }
  ...
  return (
    <div className="manual-crop-container">
      ...
      <canvas
        ref={canvasRef}
        style={{ border: '2px solid #333', maxWidth: '100%', height: 'auto' }} // Maintain aspect ratio
